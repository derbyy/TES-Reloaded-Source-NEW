	...
	if (TheShaderManager->ShaderConst.currentWorldSpace) {
		if (ShadowMapVertex && ShadowMapPixel) {
			...
			if (SunDir->z < 0.1f) {
				...
				TheShaderManager->ShaderConst.Shadow_Data.z += log(SunDir->z) / -10.0f;
...
TESObjectREFR*	ShadowManager::GetRef(TESObjectREFR* Ref) { return NULL; }
TESObjectREFR*	ShadowManager::GetRefO(TESObjectREFR* Ref) { return NULL; }
void			ShadowManager::RenderObject(NiAVObject* Node, bool HasWater) {}
void			ShadowManager::Render(NiGeometry* Geo) {}
void			ShadowManager::RenderShadowMap() {}
#endif