				...
				if (ExtendData = Data->extendData) {
					for (TList<ExtraDataList>::Iterator Itr = ExtendData->Begin(); !Itr.End() && Itr.Get(); ++Itr) {
						...
						if (DataList->IsWorn()) {
							...
							if (Form->formType == TESForm::FormType::kFormType_Weapon && DataList->HasType(BSExtraData::ExtraDataType::kExtraData_WornLeft)) LeftWeapon[From - Normal] = (TESObjectWEAP*)Form;
